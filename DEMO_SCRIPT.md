...
- Open the SPCS app URL (check with `SHOW ENDPOINTS IN SERVICE ANDE_DB.PUBLIC.ANDE_DB_SERVICE`)
...
FROM ANDE_DB.PUBLIC.WEB_ACTIVITY
...
**Action:** Open Snowsight → Data → ANDE_DB → Show the schema layers:
...
SHOW DBT PROJECTS IN SCHEMA ANDE_DB.PUBLIC;

-- Execute the pipeline
EXECUTE DBT PROJECT ANDE_DB.PUBLIC.C360_DBT ARGS='run';
```
...
FROM ANDE_DB.PUBLIC_CLEANSED.CLEANSED_SAP_CUSTOMERS;
```
...
FROM ANDE_DB.PUBLIC_MATCHED.MATCH_CLUSTERS_JARO
...
**Action:** Show lineage (Data → ANDE_DB → PUBLIC_MATCHED → MATCH_CLUSTERS_JARO → Lineage tab):
