...
const VERIFIED_QUERIES: Record<string, string> = {
  "what is the total value of all active contracts?": `SELECT SUM(CONTRACT_VALUE) AS TOTAL_ACTIVE_CONTRACT_VALUE FROM CUSTOMER_360.PUBLIC.CUSTOMER_CONTRACTS WHERE STATUS = 'Active'`,
  "show me contracts expiring in the next 30 days": `SELECT CONTRACT_ID, CUSTOMER_NAME, CONTRACT_TITLE, TO_CHAR(EXPIRY_DATE, 'DD Mon YYYY') AS EXPIRY_DATE, CONTRACT_VALUE, STATUS FROM CUSTOMER_360.PUBLIC.CUSTOMER_CONTRACTS WHERE EXPIRY_DATE BETWEEN CURRENT_DATE AND DATEADD(day, 30, CURRENT_DATE) ORDER BY EXPIRY_DATE LIMIT 20`,
  "which contracts have the highest value?": `SELECT CONTRACT_ID, CUSTOMER_NAME, CONTRACT_TITLE, CONTRACT_VALUE, STATUS, TO_CHAR(CONTRACT_DATE, 'DD Mon YYYY') AS CONTRACT_DATE FROM CUSTOMER_360.PUBLIC.CUSTOMER_CONTRACTS ORDER BY CONTRACT_VALUE DESC LIMIT 20`,
  "what is the sentiment breakdown across all calls?": `SELECT SENTIMENT, COUNT(*) AS CALL_COUNT FROM CUSTOMER_360.PUBLIC.CUSTOMER_CALLS GROUP BY SENTIMENT ORDER BY CALL_COUNT DESC`,
  "how many calls had negative sentiment?": `SELECT COUNT(*) AS NEGATIVE_CALL_COUNT FROM CUSTOMER_360.PUBLIC.CUSTOMER_CALLS WHERE SENTIMENT = 'Negative'`,
  "show me top 10 customers by contract value": `SELECT c.CUSTOMER_NAME, SUM(c.CONTRACT_VALUE) AS TOTAL_CONTRACT_VALUE, COUNT(*) AS CONTRACT_COUNT FROM CUSTOMER_360.PUBLIC.CUSTOMER_CONTRACTS c GROUP BY c.CUSTOMER_NAME ORDER BY TOTAL_CONTRACT_VALUE DESC LIMIT 10`,
  "which customers have the most source records?": `SELECT MASTER_CUSTOMER_ID, FULL_NAME, RECORD_COUNT, EMAIL, CITY FROM CUSTOMER_360.PUBLIC.CUSTOMER_MASTER_GOLDEN_TABLE ORDER BY RECORD_COUNT DESC LIMIT 20`,
  "which customers have negative call sentiment?": `SELECT DISTINCT g.FULL_NAME, g.EMAIL, c.CALL_DATE, c.SUMMARY FROM CUSTOMER_360.PUBLIC.CUSTOMER_CALLS c JOIN CUSTOMER_360.PUBLIC.CUSTOMER_MASTER_GOLDEN_TABLE g ON c.MASTER_CUSTOMER_ID = g.MASTER_CUSTOMER_ID WHERE c.SENTIMENT = 'Negative' ORDER BY c.CALL_DATE DESC LIMIT 20`,
  "which customers have expired contracts?": `SELECT DISTINCT c.CUSTOMER_NAME, c.CONTRACT_TITLE, TO_CHAR(c.EXPIRY_DATE, 'DD Mon YYYY') AS EXPIRY_DATE, c.CONTRACT_VALUE FROM CUSTOMER_360.PUBLIC.CUSTOMER_CONTRACTS c WHERE c.STATUS = 'Expired' ORDER BY c.EXPIRY_DATE DESC LIMIT 20`,
  "find all contracts signed by colm moynihan": `SELECT CONTRACT_ID, CONTRACT_TITLE, TO_CHAR(CONTRACT_DATE, 'DD Mon YYYY') AS CONTRACT_DATE, TO_CHAR(EXPIRY_DATE, 'DD Mon YYYY') AS EXPIRY_DATE, CONTRACT_VALUE, STATUS FROM CUSTOMER_360.PUBLIC.CUSTOMER_CONTRACTS WHERE UPPER(SIGNED_BY_CUSTOMER) LIKE '%MOYNIHAN%' OR UPPER(CUSTOMER_NAME) LIKE '%MOYNIHAN%' ORDER BY CONTRACT_DATE DESC LIMIT 20`,
}
